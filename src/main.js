...
function renderCart() {
  ...
  let anyTrial = false
  let anyOffline = false      // recurring invoiced by Apex outside Stripe checkout
  cart.forEach(id => {
    const p = products[id]
    if (!p) return
    totalUpfront += p.upfront
    totalMonthly += p.monthly
    if (p.monthly > 0 && !p.monthlyTrialDays && !p.monthlyOffline) monthlyDueToday += p.monthly
    if (p.monthly > 0 && p.monthlyTrialDays) anyTrial = true
    if (p.monthly > 0 && p.monthlyOffline) anyOffline = true
    ...
    if (p.upfront > 0 && p.monthly > 0) {
      ...
      const startNow = (p.monthlyTrialDays || p.monthlyOffline) ? p.upfront : p.upfront + p.monthly
  ...
  if (cartNoteEl) {
    if (totalMonthly > 0) {
      cartNoteEl.textContent = anyTrial
        ...
        : anyOffline
          ? 'Today covers your start. Your monthly is invoiced separately by Apex after that. Cancel anytime.'
